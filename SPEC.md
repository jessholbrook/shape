...
**New sign-in** (2026-09-11). A consumer email app; a sign-in from an unknown device in another country an hour ago; the notification the user sees. The best reply says what happened, where, and when, gives the harmless case and the urgent one in that order, and names the two actions. The calm reply — *no action needed unless something looks off* — is ranked last: it tells the user to do nothing about a sign-in the product can't vouch for, and if that wasn't them the account is gone while they feel looked after. The trap is a criterion nobody argues with: **reassurance**. Scored honestly it *crowns the wrong output* rather than pulling the wrong way — right that the alarmist reply is bad, wrong that the calm one is best — and lifts the calm reply to second on the totals. The Part I rubric already lets it past the alarmist one, because it wins on every criterion about how a notice sounds and loses on every one about what it says; only actionability separates the set on its own. A criterion for telling the user what happened and what to do separates them, and with completeness and actionability it reaches every pair — keep clarity and the calm reply draws level again. The lesson: reassurance measures how the product feels, not what the user can do.
...
- **More sets.** Four are seeded, each with a different trap: a criterion you'd add, one you'd think you need, one you already have, one nobody argues with. A fifth is one more entry in the set registry with no other change.
